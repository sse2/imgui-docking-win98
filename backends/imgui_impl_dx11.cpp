...
unsigned char vertex_shader[876] = {
...
unsigned char pixel_shader[660] = {
    0x44, 0x58, 0x42, 0x43, 0xD3, 0x48, 0x68, 0x07, 0xE6, 0xEA, 0x1C, 0x4F,
    0xF7, 0xBA, 0xDC, 0x67, 0xAF, 0x0F, 0x07, 0x23, 0x01, 0x00, 0x00, 0x00,
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (bd->pd3dDevice->CreateVertexShader(vertex_shader, sizeof(vertex_shader), nullptr, &bd->pVertexShader) != S_OK)
        {
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, vertex_shader, sizeof(vertex_shader), &bd->pInputLayout) != S_OK)
        {
            // vertexShaderBlob->Release();
            return false;
        }
    ...
    // Create the pixel shader
    {
        ...
        //     return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
        if (bd->pd3dDevice->CreatePixelShader(pixel_shader, sizeof(pixel_shader), nullptr, &bd->pPixelShader) != S_OK)
        {
